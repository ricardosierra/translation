<?php

return [
    'welcome'                => '歡迎來到 Facilitador Compass。每個應用都需要一個指南針來指向正確的方向。
     在本節中，您將找到許多資源和管理任務，以幫助您在建立應用時為您提供方向。',
    'links'         => [
        'title'                 => '連接',
        'documentation'         => '文件',
        'facilitador_homepage'      => 'Facilitador 官網',
        'facilitador_hooks'         => 'Facilitador Hooks',
    ],
    'commands'      => [
        'title'                 => '指令',
        'text'                  => '透過 Facilitador 運行 Artisan 指令',
        'clear_output'          => '清除輸出',
        'command_output'        => 'Artisan 指令輸出',
        'additional_args'       => '多餘參數？',
        'run_command'           => '運行指令',
    ],
    'resources'     => [
        'title'                 => '資源',
        'text'                  => 'Facilitador 資源幫助您更快找到需要的東西',

    ],
    'logs'          => [
        'title'                 => 'Logs',
        'text'                  => '您的 app logs',
        'file_too_big'          => 'Log file 大於 50MB，請下載',
        'level'                 => '等級',
        'context'               => '內容',
        'date'                  => '日期',
        'content'               => '內容',
        'download_file'         => '下載檔案',
        'delete_file'           => '刪除檔案',
        'delete_all_files'      => '刪除所有檔案',
        'delete_success'        => '成功刪除日誌檔:',
        'delete_all_success'    => '成功刪除所有日誌檔',

    ],
    'fonts'         => [
        'title'                 => '字體',
        'font_class'            => 'Facilitador Fonts Class Mapping',
        'font_character'        => 'Facilitador Fonts Character Mapping',
    ],
];
